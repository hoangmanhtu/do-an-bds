...
class functions{
	...
	public function show_tags($tags,$lass = "tags"){
		if($tags=="") return "";
		global $db;
		$arr = explode(",", $tags);
		for ($i=0,$count=count($arr); $i < $count; $i++) { 
			$row  =  $this->db->row("select name_vi,slug from #_tags where id=".$arr[$i]);
			echo '<a href="tags/'.$row["slug"].'" class="'.$lass.'">#'.$row["name_vi"].'</a>';
		}
	}
	...
	public function danhmuccap($tbl='cate',$com='',$type='',$cap='',$noibat=0){

		$this->db->bindMore(array("shows"=>1,"type"=>$type));
		$row_list  =  $this->db->query("select id,slug,name_".$this->lang." from #_".$tbl."_list where shows=:shows and type=:type order by number,id desc");
		if($com==''){
			$com = '';
		} else {
			$com = $com.'/';
		                ...
		                for($j=0;$j<count($row_cat);$j++){
		                	...
		                	if($cap>=3){
				                ...
				                for($e=0;$e<count($row_item);$e++){
				                	$data .= '<li><a href="'.$com.$row_list[$i]['slug'].'/'.$row_cat[$j]['slug'].'/'.$row_item[$e]['slug'].'">'.$row_item[$e]['name_'.$this->lang].'</a>';
				                	...
				                	if($cap>=4){
						                ...
						                for($s=0;$s<count($row_sub);$s++){
						                	$data .= '<li><a href="'.$com.$row_list[$i]['slug'].'/'.$row_cat[$j]['slug'].'/'.$row_item[$e]['slug'].'/'.$row_sub[$s]['slug'].'">'.$row_sub[$s]['name_'.$this->lang].'</a></li>';
						                }
						              	$data .= '</ul>';
					              	} }
					              	$data .= '</li>';
	...
	public function paging_home($r, $url='', $curPg=1, $mxR=5, $mxP=5, $class_paging='')
		{
			if($curPg<1) $curPg=1;
			if($mxR<1) $mxR=5;
			if($mxP<1) $mxP=5;
