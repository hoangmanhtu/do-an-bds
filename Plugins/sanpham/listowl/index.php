<?php
  ...
  $product_list = $db->query("select * from #_cate_cat where shows=:shows and type=:type and id_list=1 order by number,id desc");

  for($j=0;$j<count($product_list);$j++){ 

  $db->bindMore(array("type"=>"product","shows"=>1));
  $product  =  $db->query("select name_$lang,description_$lang,slug,photo,oldprice,price,id,thumb from #_product where shows=:shows and type=:type and id_cat='".$product_list[$j]['id']."' order by number,id desc limit 0,20");
?>
