...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.html?com=project&act=man<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"><span>Qun lý <?=$title_main ?></span></a></li>
        	<?php if($_GET['keyword']!=''){ ?>
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable SearchTable" id="checkAll">
    <thead>
      <tr>
        ...
        <td class="tb_data_small" style="width:100px">Lưt xem</td>
        <?php } ?>
    ...
    <tbody>
        ...
        <td align="center">
            <input type="text" value="<?=$items[$i]['number']?>" class="tipS smallText update_stt" original-title="Nhp số thứ tự sản phẩm" data-id="<?=$items[$i]['id']?>" data-table="<?=$com?>" data-type="stt" />
        </td> 
        ...
        <?php if(in_array('image',$config_open)){ ?>
          <td align="center"><img src="<?=_upload_project.$items[$i]['thumb']?>" width="60" /></td>
        <?php } ?>
