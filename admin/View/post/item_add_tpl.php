<?php
  //------------ tags-------------------------
  	if($item['tags']!=''){
		...
		$sql = "select id,name_vi from #_project where id<>'".$tags[0]."'";
		for ($i=1,$count = count($tags); $i < $count ; $i++) { 
			$sql .=" and id<> '".$tags[$i]."'";
		}
	} else {
		$sql = "select id,name_vi from #_project";
	}
...
<form name="supplier" id="validate" class="form" action="index.html?com=post&act=save<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
	<div class="widget">
		...
		<div class="formRow">
			<label>Mã SP  </label>
			<div class="formRight">
                <input type="text" name="code" title="Nhập mã sản phẩm" id="code" class="tipS validate[required]" value="<?=@$item['code']?>" />
			</div>
			<div class="clear"></div>
		</div>
		<?php } ?>
		...
		<div class="formRow">
			...
			<div class="formRight">
            	...
            	<?php  for ($i=0,$count = count($tags); $i < $count ; $i++) { 
        			$tags_name = $db->row("select id,name_vi from #_project where id='".$tags[$i]."'");
        			?>
