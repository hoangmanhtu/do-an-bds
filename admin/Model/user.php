...
function save_item(){
	...
	if($_POST['oldpassword']!=""){
		$data['password'] = $func->mdd5($_POST['oldpassword']);
	}
...
function edit(){
	...
	if(!empty($_POST)){
		...
		$row = $db->row("select * from #_user where username=:username");
		if($row){
			if($row['password'] != $func -> mdd5($_POST['oldpassword'])) transfer("Mật khẩu không chính xác","index.html?com=user&act=admin_edit");
		} else {
		...
		$data['username'] = $_POST['username'];
		if($_POST['new_pass']!=""){
			$data['password'] = $func -> mdd5($_POST['new_pass']);
		}
		$data['name'] = $_POST['name'];
		$data['email'] = $_POST['email'];
		$data['phone'] = $_POST['phone'];
