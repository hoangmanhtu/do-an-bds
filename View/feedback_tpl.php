    		   ...
    		   <div id="gallery-wrap">
                    <ul class="grid effect-1" id="grid">
                        <?php for($i=0,$count_ab=count($album_images);$i<$count_ab;$i++){?>
                            <li class="loaded">
                                <div class="rel"><a rel="prettyPhoto[gallery<?=$i+1?>]" title="<?=$album_detail[0]['ten_'.$lang]?>" href="<?=_upload_baiviet_l.$album_images[$i]['photo']?>"><img src="<?=_upload_baiviet_l.$album_images[$i]['photo']?>" alt="<?=$album_detail[0]['ten_'.$lang]?>"></a>
                                </div>
                            </li>
                        <?php } ?>
                    </ul>
