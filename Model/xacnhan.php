...
if(!empty($_POST)){
	...
	$dieukhoan_muahang  =  $db->row("select content_$lang from #_info where type='giaohang' ");
	$shipping  =  $db->query("select * from #_title  where shows=1 and type='shiping' order by number,id desc");
		...
		<table border="1" bordercolor="#0099CC" align="center" cellpadding="5px" cellspacing="5px" width="100%">';
			if(is_array($_SESSION['cart']))
			{
				...
				for($i=0;$i<$max;$i++){
					...
					if($q==0) continue;
            		$body.='<tr><td>'.($i+1).'</td>';
					$body.='<td> <a href="'.$config['config_base'].'/san-pham/'.$pid.'/'.changeTitle($pname).'.html" target="_blank"><img src="'.$config['config_base'].'/upload/product/'.get_thumb($pid).'" width="70" /></a></td>';
            		$body.='<td><a href="'.$config['config_base'].'/san-pham/'.$pid.'/'.changeTitle($pname).'.html" target="_blank">'.$pname.'</a></td>';
